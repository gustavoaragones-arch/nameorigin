# NameOrigin — Project Intelligence Report

_Generated 2026-07-21T18:24:59.964Z by scripts/audit/report.js (Phase 1A — read-only). Regenerate with `node scripts/audit/run-all.js`._
...
The site currently ships **7,832 generated HTML pages** built from **19 JSON datasets** by **77 Node.js scripts**. The core content unit is the first name: **3,697 name-detail pages**, each cross-linked into a dense web of related-name, surname-compatibility, sibling-pairing, and cross-country comparison pages.
...
One script dominates layer 2: **`scripts/generate-programmatic-pages.js`** is 3,679 lines and single-handedly generates name-detail pages, names-like pages, letter/gender/country/style pages, last-name filter pages, the compatibility tool page, and the 8 root hub pages. It is the largest script in the repo by a wide margin (the next-largest, `scripts/build-origin-seed.js`, is well under a third of the size) and is the single most structurally important file if you need to understand or change how most of the site is built.
...
- `scripts/clean-rebuild-3.3e.js` — deletes prior output and regenerates it via 18 steps. Its output shape (`name/`, `names/`, `compare/`, `baby-names-with-*/`, etc.) matches what is actually committed in the repo today, which makes it the de facto current pipeline.
- `scripts/build-all.js` — an older 5-step (now effectively 11-step) sequence whose first three steps write to a `/programmatic/` directory. That directory does not exist anywhere in the repo, and `generate-programmatic-pages.js` explicitly comments that "`/programmatic/` must not exist" (Phase 3.3D). `build-all.js` appears to predate that consolidation and was not updated.
...
2. `scripts/build-origin-seed.js` may add a curated entry to `data/origin-overrides.json` (currently 167 of names have one).
...
There are 19 JSON datasets in `/data`, with clear producer/consumer relationships (see `audit/datasets.json` for the full, code-derived map). The load-bearing spine is:
...
**Notable finding:** 3 datasets — `cultural-explanation-variants.json`, `comparison-intro-variants.json`, `delta-interpretation-variants.json` — produced no hits when every script in the repo was searched for their filename. They may be an early scaffold for a feature that was never wired up, or read by tooling outside this repo; nothing in `/scripts` currently appears to load them.
...
| --- | --- |
| Name pages | 3,697 |
| Names Like pages | 3,697 |
| Surname pages | 150 |
| Sibling Harmony pages | 150 |
| Equivalent-name pages | 27 |
| Letter (A–Z) pages | 26 |
| Pair pages (comparisons) | 25 |
| Country pages | 20 |
| Hub pages | 15 |
| Utility pages | 7 |
| Style pages | 7 |
| Static pages | 4 |
| Template sources (excluded from live page count) | 4 |
| Gender filter pages | 3 |
| Popularity pages | 3 |
| Trend pages | 1 |

Total generated pages: **7,832** across 24 distinct templates (see `audit/templates.json` for section-by-section, schema-by-schema detail on each one).

## 6. Where complexity exists

- **One file, most of the site.** `generate-programmatic-pages.js` (3,679 lines, ~80 functions) is the generator for name pages, names-like pages, letter/gender/country/style pages, last-name filter pages, the compatibility tool, and all 8 root hub pages. A change here has a very wide blast radius.
...
- **The classification/URL system is unambiguous.** Every one of the 7,836 HTML files on disk matched exactly one rule in a 27-rule path classifier with zero unclassified pages outside of the 4 template source files (see `audit/templates.json` coverageCheck) — the URL space, while large, is not chaotic.
...
- **Popularity data covers 5 of 3,697 names** (data/popularity.json has 7 rows total), yet a "Popularity" section is code in every name-detail page and is one of four similarity dimensions on every names-like page.
- **Origin/language data covers 4.4% of names**, and the structured **meaning field covers 0.08% of names** — both are referenced throughout the name-detail and names-like templates' copy.
