#!/usr/bin/env node
/**
 * Phase 15B Wave 2A Batch 23 — Knowledge Record creation audit.
 *
 * Usage:
 *   node scripts/build/run-phase15b-wave2-batch23-audit.js
 *   node scripts/build/run-phase15b-wave2-batch23-audit.js --apply
 */
...
const {
  ...
  BATCH23_SELECTION,
} = require('../editorial/phase15b-wave2-batch23-curated-data.js');
const { POPULARITY_PATHS } = require('../editorial/popularity-records-v1.js');

const ROOT = path.join(__dirname, '..', '..');
const AUDIT_DIR = path.join(ROOT, 'audit');
const OUT_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch23.json');
const BASELINE_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch23-baseline.json');
const PREVIOUS_BASELINE_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch22-baseline.json');
...
const WAVE2_BATCH22_KR_BASELINE = 3350;
const BATCH_SIZE = 100;
const TARGET_KR_COUNT = 3450;
const TARGET_UNRESEARCHED = 247;
...
function captureBaselineIfMissing() {
  ...
  const baseline = {
    ...
    wave2Batch22EndState: true,
    previousBaselineReference: 'audit/phase15b-wave2-batch22-baseline.json',
...
function main() {
  ...
  const batchKeys = new Set(PHASE15B_WAVE2_BATCH23_RECORDS.map((row) => normalizeKey(row.name)));
  const overlapBefore = [...batchKeys].filter((key) => preApplyKeys.has(key));

  if (doApply) {
    runNodeScript('scripts/editorial/build-phase15b-wave2-batch23-research.js');
    runNodeScript('scripts/editorial/apply-phase15b-wave2-batch23-research.js');
  }
  ...
  const batchNames = PHASE15B_WAVE2_BATCH23_RECORDS.map((row) => row.name).sort((a, b) =>
    a.localeCompare(b),
  );
  ...
  const report = {
    ...
    subwave: '2A',
    batch: 23,
    title: 'Knowledge Record Expansion Wave 2A Batch 23 — Record Creation',
    ...
    batchCreation: {
      curatedCount: PHASE15B_WAVE2_BATCH23_RECORDS.length,
      names: batchNames,
    ...
    validation: {
      ...
      cumulativeBaselinePreserved:
        beforeSummary.knowledgeRecords === WAVE2_BATCH22_KR_BASELINE || !doApply,
    },
  };

  fs.writeFileSync(OUT_PATH, JSON.stringify(report, null, 2));

  console.log('Phase 15B Wave 2A Batch 23 audit complete.');
