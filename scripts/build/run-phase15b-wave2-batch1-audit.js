#!/usr/bin/env node
/**
 * Phase 15B Wave 2A Batch 1 — Knowledge Record creation audit.
 *
 * Usage:
 *   node scripts/build/run-phase15b-wave2-batch1-audit.js
 *   node scripts/build/run-phase15b-wave2-batch1-audit.js --apply
 */
...
const {
  PHASE15B_WAVE2_BATCH1_RECORDS,
  BATCH1_SELECTION,
} = require('../editorial/phase15b-wave2-batch1-curated-data.js');
const { POPULARITY_PATHS } = require('../editorial/popularity-records-v1.js');

const ROOT = path.join(__dirname, '..', '..');
const AUDIT_DIR = path.join(ROOT, 'audit');
const OUT_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch1.json');
const BASELINE_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch1-baseline.json');
const WAVE1_BASELINE_PATH = path.join(AUDIT_DIR, 'phase15b-wave1-batch14-baseline.json');
...
const TARGET_KR_COUNT = 1250;
const TARGET_UNRESEARCHED = 2447;
...
function main() {
  ...
  const batchKeys = new Set(PHASE15B_WAVE2_BATCH1_RECORDS.map((row) => normalizeKey(row.name)));
  const overlapBefore = [...batchKeys].filter((key) => preApplyKeys.has(key));

  if (doApply) {
    runNodeScript('scripts/editorial/build-phase15b-wave2-batch1-research.js');
    runNodeScript('scripts/editorial/apply-phase15b-wave2-batch1-research.js');
  }
  ...
  const batchNames = PHASE15B_WAVE2_BATCH1_RECORDS.map((row) => row.name).sort((a, b) =>
    a.localeCompare(b),
  );
  ...
  const report = {
    ...
    batchCreation: {
      curatedCount: PHASE15B_WAVE2_BATCH1_RECORDS.length,
      names: batchNames,
    ...
    validation: {
      ...
      wave1BaselinePreserved: beforeSummary.knowledgeRecords === WAVE1_KR_BASELINE || !doApply,
    },
  };

  fs.writeFileSync(OUT_PATH, JSON.stringify(report, null, 2));

  console.log('Phase 15B Wave 2A Batch 1 audit complete.');
