#!/usr/bin/env node
/**
 * Phase 15B Wave 2A Batch 6 — Knowledge Record creation audit.
 *
 * Usage:
 *   node scripts/build/run-phase15b-wave2-batch6-audit.js
 *   node scripts/build/run-phase15b-wave2-batch6-audit.js --apply
 */
...
const {
  ...
  BATCH6_SELECTION,
} = require('../editorial/phase15b-wave2-batch6-curated-data.js');
const { POPULARITY_PATHS } = require('../editorial/popularity-records-v1.js');

const ROOT = path.join(__dirname, '..', '..');
const AUDIT_DIR = path.join(ROOT, 'audit');
const OUT_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch6.json');
const BASELINE_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch6-baseline.json');
const PREVIOUS_BASELINE_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch5-baseline.json');
...
const WAVE2_BATCH5_KR_BASELINE = 1650;
const BATCH_SIZE = 100;
const TARGET_KR_COUNT = 1750;
const TARGET_UNRESEARCHED = 1947;
...
function captureBaselineIfMissing() {
  ...
  const baseline = {
    ...
    wave2Batch5EndState: true,
    previousBaselineReference: 'audit/phase15b-wave2-batch5-baseline.json',
...
function main() {
  ...
  const batchKeys = new Set(PHASE15B_WAVE2_BATCH6_RECORDS.map((row) => normalizeKey(row.name)));
  const overlapBefore = [...batchKeys].filter((key) => preApplyKeys.has(key));

  if (doApply) {
    runNodeScript('scripts/editorial/build-phase15b-wave2-batch6-research.js');
    runNodeScript('scripts/editorial/apply-phase15b-wave2-batch6-research.js');
  }
  ...
  const batchNames = PHASE15B_WAVE2_BATCH6_RECORDS.map((row) => row.name).sort((a, b) =>
    a.localeCompare(b),
  );
  ...
  const report = {
    ...
    subwave: '2A',
    batch: 6,
    title: 'Knowledge Record Expansion Wave 2A Batch 6 — Record Creation',
    ...
    batchCreation: {
      curatedCount: PHASE15B_WAVE2_BATCH6_RECORDS.length,
      names: batchNames,
    ...
    validation: {
      ...
      cumulativeBaselinePreserved:
        beforeSummary.knowledgeRecords === WAVE2_BATCH5_KR_BASELINE || !doApply,
    },
  };

  fs.writeFileSync(OUT_PATH, JSON.stringify(report, null, 2));

  console.log('Phase 15B Wave 2A Batch 6 audit complete.');
