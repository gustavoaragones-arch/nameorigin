#!/usr/bin/env node
/**
 * Phase 15B Wave 1 Batch 14 — editorial completion audit.
 *
 * Usage:
 *   node scripts/build/run-phase15b-wave1-batch14-audit.js
 *   node scripts/build/run-phase15b-wave1-batch14-audit.js --apply
 */
...
const {
  ...
  BATCH14_SELECTION,
} = require('../editorial/phase15b-wave1-batch14-curated-data.js');
const { POPULARITY_PATHS } = require('../editorial/popularity-records-v1.js');

const ROOT = path.join(__dirname, '..', '..');
const AUDIT_DIR = path.join(ROOT, 'audit');
const OUT_PATH = path.join(AUDIT_DIR, 'phase15b-wave1-batch14.json');
const BASELINE_PATH = path.join(AUDIT_DIR, 'phase15b-wave1-batch14-baseline.json');
...
const WAVE1_CUMULATIVE_TARGET = 1150;
...
function main() {
  ...
  if (doApply) {
    runNodeScript('scripts/editorial/build-phase15b-wave1-batch14-research.js');
    runNodeScript('scripts/editorial/apply-phase15b-wave1-batch14-research.js');
  }
  ...
  const batchNames = PHASE15B_WAVE1_BATCH14_RECORDS.map((row) => row.name).sort((a, b) =>
    a.localeCompare(b),
  );
  ...
  const report = {
    ...
    wave: 1,
    batch: 14,
    title: 'Knowledge Record Expansion Wave 1 Batch 14 — Final Domain Completion',
    ...
    wave1Progress: {
      ...
      wave1Complete: afterEditorial.overall?.partialKnowledgeRecords === 0,
    },
    governance: {
      schemaFrozen: true,
    ...
    batchCompletion: {
      curatedCount: PHASE15B_WAVE1_BATCH14_RECORDS.length,
      names: batchNames,
      targetDomains: ['origin', 'meaning'],
    },
    qaStatus: {
    ...
    validation: {
      ...
      wave1FullyComplete:
        afterEditorial.overall?.fullyResearchedEntities === 1150 &&
        afterEditorial.overall?.partialKnowledgeRecords === 0,
    },
  };

  fs.writeFileSync(OUT_PATH, JSON.stringify(report, null, 2));

  console.log('Phase 15B Wave 1 Batch 14 audit complete.');
  ...
  if (report.validation.wave1FullyComplete) {
    runNodeScript('scripts/build/run-phase15b-wave1-closeout.js');
  }
}
