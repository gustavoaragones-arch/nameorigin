#!/usr/bin/env node
/**
 * Phase 15B Wave 2A Batch 7 — Knowledge Record creation audit.
 *
 * Usage:
 *   node scripts/build/run-phase15b-wave2-batch7-audit.js
 *   node scripts/build/run-phase15b-wave2-batch7-audit.js --apply
 */
...
const {
  PHASE15B_WAVE2_BATCH7_RECORDS,
  BATCH7_SELECTION,
} = require('../editorial/phase15b-wave2-batch7-curated-data.js');
const { POPULARITY_PATHS } = require('../editorial/popularity-records-v1.js');

const ROOT = path.join(__dirname, '..', '..');
const AUDIT_DIR = path.join(ROOT, 'audit');
const OUT_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch7.json');
const BASELINE_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch7-baseline.json');
const PREVIOUS_BASELINE_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch6-baseline.json');
...
const WAVE2_BATCH6_KR_BASELINE = 1750;
const BATCH_SIZE = 100;
const TARGET_KR_COUNT = 1850;
const TARGET_UNRESEARCHED = 1847;
...
function captureBaselineIfMissing() {
  ...
  const baseline = {
    ...
    wave2Batch6EndState: true,
    previousBaselineReference: 'audit/phase15b-wave2-batch6-baseline.json',
...
function main() {
  ...
  const batchKeys = new Set(PHASE15B_WAVE2_BATCH7_RECORDS.map((row) => normalizeKey(row.name)));
  const overlapBefore = [...batchKeys].filter((key) => preApplyKeys.has(key));

  if (doApply) {
    runNodeScript('scripts/editorial/build-phase15b-wave2-batch7-research.js');
    runNodeScript('scripts/editorial/apply-phase15b-wave2-batch7-research.js');
  }
  ...
  const batchNames = PHASE15B_WAVE2_BATCH7_RECORDS.map((row) => row.name).sort((a, b) =>
    a.localeCompare(b),
  );
  ...
  const report = {
    ...
    subwave: '2A',
    batch: 7,
    title: 'Knowledge Record Expansion Wave 2A Batch 7 — Record Creation',
    generatedAt: new Date().toISOString(),
    baselineReference: 'editorial-architecture-v2',
    governanceReference: 'audit/phase16b-governance-check.json',
    cumulativeBaselineReference: 'audit/phase15b-wave2-batch6-baseline.json',
    batchSelection: BATCH7_SELECTION,
    wave2Progress: {
      cumulativeKrTarget: TARGET_KR_COUNT,
      cumulativeKrActual: afterSummary.knowledgeRecords,
      batchesComplete: doApply ? 7 : 6,
    ...
    batchCreation: {
      curatedCount: PHASE15B_WAVE2_BATCH7_RECORDS.length,
      names: batchNames,
    ...
    validation: {
      ...
      cumulativeBaselinePreserved:
        beforeSummary.knowledgeRecords === WAVE2_BATCH6_KR_BASELINE || !doApply,
    },
  };

  fs.writeFileSync(OUT_PATH, JSON.stringify(report, null, 2));

  console.log('Phase 15B Wave 2A Batch 7 audit complete.');
