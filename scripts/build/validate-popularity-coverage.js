#!/usr/bin/env node
/**
 * Phase 14A — Validate popularity coverage intelligence outputs.
 */
...
function main() {
  ...
  if (!fs.existsSync(COVERAGE_PATH)) errors.push('Missing audit/popularity-coverage.json');
  if (!fs.existsSync(GAP_PATH)) errors.push('Missing audit/popularity-gap-analysis.json');
  ...
  if (JSON.stringify(coverageA) !== JSON.stringify(coverageB)) {
    errors.push('Popularity coverage report is not deterministically reproducible.');
  }
  ...
  if (JSON.stringify(gapA) !== JSON.stringify(gapB)) {
    errors.push('Popularity gap analysis is not deterministically reproducible.');
  }
  ...
  console.log('Popularity coverage validation:', errors.length === 0 ? 'PASS' : 'FAIL');
  if (errors.length) {
    errors.slice(0, 20).forEach((msg) => console.error('  -', msg));
    process.exitCode = 1;
    throw new Error('Popularity coverage validation failed.');
  }
}
