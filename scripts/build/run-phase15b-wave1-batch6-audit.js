#!/usr/bin/env node
/**
 * Phase 15B Wave 1 Batch 6 — editorial completion audit.
 *
 * Usage:
 *   node scripts/build/run-phase15b-wave1-batch6-audit.js
 *   node scripts/build/run-phase15b-wave1-batch6-audit.js --apply
 */
...
const {
  ...
  BATCH6_SELECTION,
} = require('../editorial/phase15b-wave1-batch6-curated-data.js');
const { POPULARITY_PATHS } = require('../editorial/popularity-records-v1.js');

const ROOT = path.join(__dirname, '..', '..');
const AUDIT_DIR = path.join(ROOT, 'audit');
const OUT_PATH = path.join(AUDIT_DIR, 'phase15b-wave1-batch6.json');
const BASELINE_PATH = path.join(AUDIT_DIR, 'phase15b-wave1-batch6-baseline.json');
...
const WAVE1_CUMULATIVE_TARGET = 950;
...
function main() {
  ...
  if (doApply) {
    runNodeScript('scripts/editorial/build-phase15b-wave1-batch6-research.js');
    runNodeScript('scripts/editorial/apply-phase15b-wave1-batch6-research.js');
  }
  ...
  const batchNames = PHASE15B_WAVE1_BATCH6_RECORDS.map((row) => row.name).sort((a, b) =>
    a.localeCompare(b),
  );
  ...
  const report = {
    ...
    wave: 1,
    batch: 6,
    title: 'Knowledge Record Expansion Wave 1 Batch 6 — Domain Completion',
    ...
    batchCompletion: {
      curatedCount: PHASE15B_WAVE1_BATCH6_RECORDS.length,
      names: batchNames,
  ...
  console.log('Phase 15B Wave 1 Batch 6 audit complete.');
