#!/usr/bin/env node
/**
 * Phase 15B Wave 2A Batch 26 — Knowledge Record creation audit (full corpus completion).
 *
 * Usage:
 *   node scripts/build/run-phase15b-wave2-batch26-audit.js
 *   node scripts/build/run-phase15b-wave2-batch26-audit.js --apply
 */
...
const {
  ...
  BATCH26_SELECTION,
} = require('../editorial/phase15b-wave2-batch26-curated-data.js');
const { POPULARITY_PATHS } = require('../editorial/popularity-records-v1.js');

const ROOT = path.join(__dirname, '..', '..');
const AUDIT_DIR = path.join(ROOT, 'audit');
const OUT_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch26.json');
const BASELINE_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch26-baseline.json');
const PREVIOUS_BASELINE_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch25-baseline.json');
...
const WAVE2_BATCH25_KR_BASELINE = 3650;
const BATCH_SIZE = 47;
const TARGET_KR_COUNT = 3697;
const TARGET_UNRESEARCHED = 0;
...
function captureBaselineIfMissing() {
  ...
  const baseline = {
    ...
    wave2Batch25EndState: true,
    previousBaselineReference: 'audit/phase15b-wave2-batch25-baseline.json',
...
function main() {
  ...
  const batchKeys = new Set(PHASE15B_WAVE2_BATCH26_RECORDS.map((row) => normalizeKey(row.name)));
  const overlapBefore = [...batchKeys].filter((key) => preApplyKeys.has(key));

  if (doApply) {
    runNodeScript('scripts/editorial/build-phase15b-wave2-batch26-research.js');
    runNodeScript('scripts/editorial/apply-phase15b-wave2-batch26-research.js');
  }
  ...
  const batchNames = PHASE15B_WAVE2_BATCH26_RECORDS.map((row) => row.name).sort((a, b) =>
    a.localeCompare(b),
  );
  ...
  const report = {
    ...
    batchCreation: {
      curatedCount: PHASE15B_WAVE2_BATCH26_RECORDS.length,
      names: batchNames,
    ...
    validation: {
      ...
      cumulativeBaselinePreserved:
        beforeSummary.knowledgeRecords === WAVE2_BATCH25_KR_BASELINE || !doApply,
      fullCorpusComplete:
        afterSummary.knowledgeRecords === ENTITY_COUNT &&
        (unresearchedAfter ?? 1) === 0,
    },
  };

  fs.writeFileSync(OUT_PATH, JSON.stringify(report, null, 2));

  console.log('Phase 15B Wave 2A Batch 26 audit complete.');
  ...
  console.log('  Wave 2 complete:', report.wave2Progress.wave2Complete);
