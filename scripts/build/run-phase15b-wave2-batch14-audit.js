#!/usr/bin/env node
/**
 * Phase 15B Wave 2A Batch 14 — Knowledge Record creation audit.
 *
 * Usage:
 *   node scripts/build/run-phase15b-wave2-batch14-audit.js
 *   node scripts/build/run-phase15b-wave2-batch14-audit.js --apply
 */
...
const {
  ...
  BATCH14_SELECTION,
} = require('../editorial/phase15b-wave2-batch14-curated-data.js');
const { POPULARITY_PATHS } = require('../editorial/popularity-records-v1.js');

const ROOT = path.join(__dirname, '..', '..');
const AUDIT_DIR = path.join(ROOT, 'audit');
const OUT_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch14.json');
const BASELINE_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch14-baseline.json');
const PREVIOUS_BASELINE_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch13-baseline.json');
...
const WAVE2_BATCH13_KR_BASELINE = 2450;
const BATCH_SIZE = 100;
const TARGET_KR_COUNT = 2550;
const TARGET_UNRESEARCHED = 1147;
...
function captureBaselineIfMissing() {
  ...
  const baseline = {
    ...
    wave2Batch13EndState: true,
    previousBaselineReference: 'audit/phase15b-wave2-batch13-baseline.json',
...
function main() {
  ...
  const batchKeys = new Set(PHASE15B_WAVE2_BATCH14_RECORDS.map((row) => normalizeKey(row.name)));
  const overlapBefore = [...batchKeys].filter((key) => preApplyKeys.has(key));

  if (doApply) {
    runNodeScript('scripts/editorial/build-phase15b-wave2-batch14-research.js');
    runNodeScript('scripts/editorial/apply-phase15b-wave2-batch14-research.js');
  }
  ...
  const batchNames = PHASE15B_WAVE2_BATCH14_RECORDS.map((row) => row.name).sort((a, b) =>
    a.localeCompare(b),
  );
  ...
  const report = {
    ...
    subwave: '2A',
    batch: 14,
    title: 'Knowledge Record Expansion Wave 2A Batch 14 — Record Creation',
    ...
    batchCreation: {
      curatedCount: PHASE15B_WAVE2_BATCH14_RECORDS.length,
      names: batchNames,
    ...
    validation: {
      ...
      cumulativeBaselinePreserved:
        beforeSummary.knowledgeRecords === WAVE2_BATCH13_KR_BASELINE || !doApply,
    },
  };

  fs.writeFileSync(OUT_PATH, JSON.stringify(report, null, 2));

  console.log('Phase 15B Wave 2A Batch 14 audit complete.');
