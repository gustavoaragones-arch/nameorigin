#!/usr/bin/env node
/**
 * Phase 7A — Wave 2 Batch 1 editorial expansion audit.
 *
...
const { WAVE2_BATCH1_RECORDS } = require('../editorial/wave2-batch1-curated-data.js');

const ROOT = path.join(__dirname, '..', '..');
const AUDIT_DIR = path.join(ROOT, 'audit');
const OUT_PATH = path.join(AUDIT_DIR, 'phase7a-wave2.json');
const BASELINE_PATH = path.join(AUDIT_DIR, 'phase7a-wave2-baseline.json');
...
function main() {
  ...
  if (doApply) {
    runNodeScript('scripts/editorial/build-wave2-batch1-research.js');
    runNodeScript('scripts/editorial/apply-wave2-batch1-research.js');
  }
  ...
  const newRecordNames = WAVE2_BATCH1_RECORDS.map((row) => row.name).sort((a, b) => a.localeCompare(b));
  ...
  const report = {
    ...
    newEditorialRecords: {
      curatedCount: WAVE2_BATCH1_RECORDS.length,
    ...
    validation: {
      ...
      targetKnowledgeRecordsApprox950: after.knowledgeRecords >= 940 && after.knowledgeRecords <= 960,
      targetCoverageApprox20Pct: DOMAINS.every(
        (domain) => after.domainCoverage[domain].pct >= 18 && after.domainCoverage[domain].pct <= 22,
      ),
  ...
  console.log('Phase 7A Wave 2 Batch 1 audit complete.');
