#!/usr/bin/env node
/**
 * Phase 15B Wave 2A Batch 13 — Knowledge Record creation audit.
 *
 * Usage:
 *   node scripts/build/run-phase15b-wave2-batch13-audit.js
 *   node scripts/build/run-phase15b-wave2-batch13-audit.js --apply
 */
...
const {
  PHASE15B_WAVE2_BATCH13_RECORDS,
  BATCH13_SELECTION,
} = require('../editorial/phase15b-wave2-batch13-curated-data.js');
const { POPULARITY_PATHS } = require('../editorial/popularity-records-v1.js');

const ROOT = path.join(__dirname, '..', '..');
const AUDIT_DIR = path.join(ROOT, 'audit');
const OUT_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch13.json');
const BASELINE_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch13-baseline.json');
const PREVIOUS_BASELINE_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch12-baseline.json');
...
const WAVE2_BATCH12_KR_BASELINE = 2350;
const BATCH_SIZE = 100;
const TARGET_KR_COUNT = 2450;
const TARGET_UNRESEARCHED = 1247;
...
function captureBaselineIfMissing() {
  ...
  const baseline = {
    ...
    wave2Batch12EndState: true,
    previousBaselineReference: 'audit/phase15b-wave2-batch12-baseline.json',
...
function main() {
  ...
  const batchKeys = new Set(PHASE15B_WAVE2_BATCH13_RECORDS.map((row) => normalizeKey(row.name)));
  const overlapBefore = [...batchKeys].filter((key) => preApplyKeys.has(key));

  if (doApply) {
    runNodeScript('scripts/editorial/build-phase15b-wave2-batch13-research.js');
    runNodeScript('scripts/editorial/apply-phase15b-wave2-batch13-research.js');
  }
  ...
  const batchNames = PHASE15B_WAVE2_BATCH13_RECORDS.map((row) => row.name).sort((a, b) =>
    a.localeCompare(b),
  );
  ...
  const report = {
    ...
    subwave: '2A',
    batch: 13,
    title: 'Knowledge Record Expansion Wave 2A Batch 13 — Record Creation',
    generatedAt: new Date().toISOString(),
    baselineReference: 'editorial-architecture-v2',
    governanceReference: 'audit/phase16b-governance-check.json',
    cumulativeBaselineReference: 'audit/phase15b-wave2-batch12-baseline.json',
    batchSelection: BATCH13_SELECTION,
    wave2Progress: {
      cumulativeKrTarget: TARGET_KR_COUNT,
      cumulativeKrActual: afterSummary.knowledgeRecords,
      batchesComplete: doApply ? 13 : 12,
      unresearchedRemaining: unresearchedAfter,
      milestone: 'Halfway governance review',
    },
    governance: {
      schemaFrozen: true,
    ...
    batchCreation: {
      curatedCount: PHASE15B_WAVE2_BATCH13_RECORDS.length,
      names: batchNames,
    ...
    validation: {
      ...
      cumulativeBaselinePreserved:
        beforeSummary.knowledgeRecords === WAVE2_BATCH12_KR_BASELINE || !doApply,
    },
  };
  ...
  if (doApply) {
    runNodeScript('scripts/build/run-phase15b-wave2-halfway-review.js');
  }

  console.log('Phase 15B Wave 2A Batch 13 audit complete.');
