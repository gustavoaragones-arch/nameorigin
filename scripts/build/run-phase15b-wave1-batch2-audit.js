#!/usr/bin/env node
/**
 * Phase 15B Wave 1 Batch 2 — editorial completion audit.
 *
 * Completes missing domains for ranks 26–50 partial Knowledge Records.
 * Editorial data only — does not modify platform architecture.
 *
 * Usage:
 *   node scripts/build/run-phase15b-wave1-batch2-audit.js
 *   node scripts/build/run-phase15b-wave1-batch2-audit.js --apply
 */
...
const { PHASE15B_WAVE1_BATCH2_RECORDS } = require('../editorial/phase15b-wave1-batch2-curated-data.js');
const { POPULARITY_PATHS } = require('../editorial/popularity-records-v1.js');

const ROOT = path.join(__dirname, '..', '..');
const AUDIT_DIR = path.join(ROOT, 'audit');
const OUT_PATH = path.join(AUDIT_DIR, 'phase15b-wave1-batch2.json');
const BASELINE_PATH = path.join(AUDIT_DIR, 'phase15b-wave1-batch2-baseline.json');
...
const WAVE1_CUMULATIVE_TARGET = 850;
...
function main() {
  ...
  if (doApply) {
    runNodeScript('scripts/editorial/build-phase15b-wave1-batch2-research.js');
    runNodeScript('scripts/editorial/apply-phase15b-wave1-batch2-research.js');
  }
  ...
  const batchNames = PHASE15B_WAVE1_BATCH2_RECORDS.map((row) => row.name).sort((a, b) =>
    a.localeCompare(b),
  );
  ...
  const report = {
    ...
    wave: 1,
    batch: 2,
    title: 'Knowledge Record Expansion Wave 1 Batch 2 — Domain Completion',
    ...
    batchCompletion: {
      curatedCount: PHASE15B_WAVE1_BATCH2_RECORDS.length,
      names: batchNames,
  ...
  console.log('Phase 15B Wave 1 Batch 2 audit complete.');
