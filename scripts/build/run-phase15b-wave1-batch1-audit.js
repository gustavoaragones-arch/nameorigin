#!/usr/bin/env node
/**
 * Phase 15B Wave 1 Batch 1 — editorial completion audit.
 *
 * Completes missing domains for ranks 1–25 partial Knowledge Records.
 * Editorial data only — does not modify platform architecture.
 *
 * Usage:
 *   node scripts/build/run-phase15b-wave1-batch1-audit.js
 *   node scripts/build/run-phase15b-wave1-batch1-audit.js --apply
 */
...
const { PHASE15B_WAVE1_BATCH1_RECORDS } = require('../editorial/phase15b-wave1-batch1-curated-data.js');
...
const OUT_PATH = path.join(AUDIT_DIR, 'phase15b-wave1-batch1.json');
const BASELINE_PATH = path.join(AUDIT_DIR, 'phase15b-wave1-batch1-baseline.json');
...
function main() {
  ...
  if (doApply) {
    runNodeScript('scripts/editorial/build-phase15b-wave1-batch1-research.js');
    runNodeScript('scripts/editorial/apply-phase15b-wave1-batch1-research.js');
  }
  ...
  const batchNames = PHASE15B_WAVE1_BATCH1_RECORDS.map((row) => row.name).sort((a, b) =>
    a.localeCompare(b),
  );
  ...
  const report = {
    ...
    editorialCoverage: {
      before: {
        fullyResearchedEntities: beforeEditorial.overall?.fullyResearchedEntities ?? before.editorialCoverage?.fullyResearchedEntities,
        partialKnowledgeRecords: beforeEditorial.overall?.partialKnowledgeRecords ?? before.editorialCoverage?.partialKnowledgeRecords,
    ...
    batchCompletion: {
      curatedCount: PHASE15B_WAVE1_BATCH1_RECORDS.length,
      names: batchNames,
      targetDomains: ['meaning', 'pronunciation', 'etymology', 'history'],
    },
    qaStatus: {
  ...
  console.log('Phase 15B Wave 1 Batch 1 audit complete.');
  ...
  DOMAINS.filter((d) => d !== 'origin').forEach((domain) => {
    console.log(
      `  ${domain}:`,
