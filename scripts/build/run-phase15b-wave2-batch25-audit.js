#!/usr/bin/env node
/**
 * Phase 15B Wave 2A Batch 25 — Knowledge Record creation audit.
 *
 * Usage:
 *   node scripts/build/run-phase15b-wave2-batch25-audit.js
 *   node scripts/build/run-phase15b-wave2-batch25-audit.js --apply
 */
...
const {
  PHASE15B_WAVE2_BATCH25_RECORDS,
  BATCH25_SELECTION,
} = require('../editorial/phase15b-wave2-batch25-curated-data.js');
const { POPULARITY_PATHS } = require('../editorial/popularity-records-v1.js');

const ROOT = path.join(__dirname, '..', '..');
const AUDIT_DIR = path.join(ROOT, 'audit');
const OUT_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch25.json');
const BASELINE_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch25-baseline.json');
const PREVIOUS_BASELINE_PATH = path.join(AUDIT_DIR, 'phase15b-wave2-batch24-baseline.json');
...
const WAVE2_BATCH24_KR_BASELINE = 3550;
const BATCH_SIZE = 100;
const TARGET_KR_COUNT = 3650;
const TARGET_UNRESEARCHED = 47;
...
function captureBaselineIfMissing() {
  ...
  const baseline = {
    ...
    wave2Batch24EndState: true,
    previousBaselineReference: 'audit/phase15b-wave2-batch24-baseline.json',
...
function main() {
  ...
  const batchKeys = new Set(PHASE15B_WAVE2_BATCH25_RECORDS.map((row) => normalizeKey(row.name)));
  const overlapBefore = [...batchKeys].filter((key) => preApplyKeys.has(key));

  if (doApply) {
    runNodeScript('scripts/editorial/build-phase15b-wave2-batch25-research.js');
    runNodeScript('scripts/editorial/apply-phase15b-wave2-batch25-research.js');
  }
  ...
  const batchNames = PHASE15B_WAVE2_BATCH25_RECORDS.map((row) => row.name).sort((a, b) =>
    a.localeCompare(b),
  );
  ...
  const report = {
    ...
    subwave: '2A',
    batch: 25,
    title: 'Knowledge Record Expansion Wave 2A Batch 25 — Record Creation',
    generatedAt: new Date().toISOString(),
    baselineReference: 'editorial-architecture-v2',
    governanceReference: 'audit/phase16b-governance-check.json',
    cumulativeBaselineReference: 'audit/phase15b-wave2-batch24-baseline.json',
    batchSelection: BATCH25_SELECTION,
    wave2Progress: {
      cumulativeKrTarget: TARGET_KR_COUNT,
      cumulativeKrActual: afterSummary.knowledgeRecords,
      batchesComplete: doApply ? 25 : 24,
    ...
    batchCreation: {
      curatedCount: PHASE15B_WAVE2_BATCH25_RECORDS.length,
      names: batchNames,
    ...
    validation: {
      ...
      cumulativeBaselinePreserved:
        beforeSummary.knowledgeRecords === WAVE2_BATCH24_KR_BASELINE || !doApply,
    },
  };

  fs.writeFileSync(OUT_PATH, JSON.stringify(report, null, 2));

  console.log('Phase 15B Wave 2A Batch 25 audit complete.');
