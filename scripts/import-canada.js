#!/usr/bin/env node
/**
 * Step 2 — Raw data pipeline: Statistics Canada.
 * Downloads CSV zip, parses, outputs normalized temp data to raw-data/canada/.
 */
...
const RAW_CANADA = path.join(ROOT, 'raw-data', 'canada');
const STATCAN_URL = 'https://www150.statcan.gc.ca/n1/tbl/csv/17100147-eng.zip';
const ZIP_PATH = path.join(RAW_CANADA, '17100147-eng.zip');
const NORMALIZED_PATH = path.join(RAW_CANADA, 'normalized.json');
...
function findCsv(dir) {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) { const f = findCsv(full); if (f) return f; }
...
async function main() {
  ...
  if (!fs.existsSync(ZIP_PATH)) {
    ...
    try {
      ...
      console.error('Manual: download', STATCAN_URL, 'and save as', ZIP_PATH);
      process.exit(1);
    }
  } else {
  ...
  execSync(`unzip -o -q "${ZIP_PATH}" -d "${RAW_CANADA}"`, { stdio: 'inherit' });

  const csvPath = findCsv(RAW_CANADA);
