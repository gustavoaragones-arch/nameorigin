#!/usr/bin/env node
/**
 * Phase 5E — Apply Wave 1 history research to data/history-overrides.json
 * and regenerate data/names-enriched.json via rebuild-names-enriched.js.
 *
 * Preserves any existing history data unchanged.
 */
...
const RESEARCH_PATH = path.join(DATA_DIR, 'sources', 'history-wave1-research.json');
const OVERRIDES_PATH = path.join(DATA_DIR, 'history-overrides.json');
...
function main() {
  ...
  if (!research || !Array.isArray(research.entries)) {
    throw new Error('Missing or invalid history-wave1-research.json');
  }
  ...
  for (const entry of research.entries) {
    ...
    if (!entry.history || !String(entry.history).trim()) {
      throw new Error(`Entry ${entry.name} has no history populated.`);
    }
  ...
  console.log('Phase 5E Wave 1 history research applied.');
  console.log('  New histories added:', added);
  console.log('  Skipped (already researched):', skippedExisting);
  console.log('  Skipped (not in names.json):', skippedMissing);
  console.log('  Total history overrides now:', Object.keys(overrides).length);
}
