#!/usr/bin/env node
/**
 * Phase 15B Wave 1 Batch 14 — build editorial research artifact.
 */
...
const {
  ...
  BATCH14_SELECTION,
} = require('./phase15b-wave1-batch14-curated-data.js');

const ROOT = path.join(__dirname, '..', '..');
const DATA_DIR = path.join(ROOT, 'data');
const SOURCES_DIR = path.join(DATA_DIR, 'sources');
const OUT_PATH = path.join(SOURCES_DIR, 'phase15b-wave1-batch14-research.json');

function main() {
  const entries = PHASE15B_WAVE1_BATCH14_RECORDS.map((record) => ({
    name: record.name,
  ...
  const payload = {
    ...
    wave: 1,
    batch: 11,
    title: 'Knowledge Record Expansion Wave 1 Batch 14 — Domain Completion',
    generatedAt: new Date().toISOString(),
    baselineReference: 'editorial-architecture-v2',
    selection: BATCH14_SELECTION,
    methodology:
  ...
  console.log('Phase 15B Wave 1 Batch 14 research file written.');
