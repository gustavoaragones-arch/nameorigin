#!/usr/bin/env node
/**
 * Phase 15B Wave 2A Batch 15 — build editorial research artifact.
 */
...
const {
  ...
  BATCH15_SELECTION,
} = require('./phase15b-wave2-batch15-curated-data.js');

const ROOT = path.join(__dirname, '..', '..');
const DATA_DIR = path.join(ROOT, 'data');
const SOURCES_DIR = path.join(DATA_DIR, 'sources');
const OUT_PATH = path.join(SOURCES_DIR, 'phase15b-wave2-batch15-research.json');

function main() {
  const entries = PHASE15B_WAVE2_BATCH15_RECORDS.map((record) => ({
    name: record.name,
  ...
  console.log('Phase 15B Wave 2A Batch 15 research file written.');
