#!/usr/bin/env node
/**
 * Phase 15B Wave 1 Batch 6 — build editorial research artifact.
 */
...
const {
  ...
  BATCH6_SELECTION,
} = require('./phase15b-wave1-batch6-curated-data.js');

const ROOT = path.join(__dirname, '..', '..');
const DATA_DIR = path.join(ROOT, 'data');
const SOURCES_DIR = path.join(DATA_DIR, 'sources');
const OUT_PATH = path.join(SOURCES_DIR, 'phase15b-wave1-batch6-research.json');

function main() {
  const entries = PHASE15B_WAVE1_BATCH6_RECORDS.map((record) => ({
    name: record.name,
  ...
  console.log('Phase 15B Wave 1 Batch 6 research file written.');
