#!/usr/bin/env node
/**
 * Phase 15B Wave 1 Batch 11 — build editorial research artifact.
 */
...
const {
  ...
  BATCH11_SELECTION,
} = require('./phase15b-wave1-batch11-curated-data.js');

const ROOT = path.join(__dirname, '..', '..');
const DATA_DIR = path.join(ROOT, 'data');
const SOURCES_DIR = path.join(DATA_DIR, 'sources');
const OUT_PATH = path.join(SOURCES_DIR, 'phase15b-wave1-batch11-research.json');

function main() {
  const entries = PHASE15B_WAVE1_BATCH11_RECORDS.map((record) => ({
    name: record.name,
  ...
  console.log('Phase 15B Wave 1 Batch 11 research file written.');
