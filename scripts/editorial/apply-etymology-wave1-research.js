#!/usr/bin/env node
/**
 * Phase 5D — Apply Wave 1 etymology research to data/etymology-overrides.json
 * and regenerate data/names-enriched.json via rebuild-names-enriched.js.
 *
 * Preserves any existing etymology data unchanged.
 */
...
const RESEARCH_PATH = path.join(DATA_DIR, 'sources', 'etymology-wave1-research.json');
const OVERRIDES_PATH = path.join(DATA_DIR, 'etymology-overrides.json');
...
function main() {
  ...
  if (!research || !Array.isArray(research.entries)) {
    throw new Error('Missing or invalid etymology-wave1-research.json');
  }
  ...
  for (const entry of research.entries) {
    ...
    if (!entry.etymology || !String(entry.etymology).trim()) {
      throw new Error(`Entry ${entry.name} has no etymology populated.`);
    }

    overrides[key] = {
      etymology: String(entry.etymology).trim(),
  ...
  console.log('Phase 5D Wave 1 etymology research applied.');
  console.log('  New etymologies added:', added);
  console.log('  Skipped (already researched):', skippedExisting);
  console.log('  Skipped (not in names.json):', skippedMissing);
  console.log('  Total etymology overrides now:', Object.keys(overrides).length);
}
