#!/usr/bin/env node
/**
 * Phase 13A — Citation gap analysis and prioritization (read-only).
 *
 * Produces audit/citation-gap-analysis.json
 */

const fs = require('fs');
const path = require('path');
const {
  buildEntityIndex,
  buildCoverageReport,
  buildGapAnalysisReport,
  AUDIT_DIR,
} = require('../../lib/analysis/citation-coverage-intelligence.js');
const { writeAuditJson } = require('./_lib.js');

function main() {
  console.log('Phase 13A — Citation Gap Analysis');

  const coveragePath = path.join(AUDIT_DIR, 'citation-coverage.json');
  const ctx = buildEntityIndex();
  const coverageReport = fs.existsSync(coveragePath)
    ? JSON.parse(fs.readFileSync(coveragePath, 'utf8'))
    : buildCoverageReport(ctx);

  const report = buildGapAnalysisReport(ctx, coverageReport);
  writeAuditJson('citation-gap-analysis.json', report);

  console.log('  Entities ranked:', report.totals.entitiesRanked);
  console.log('  Zero-citation entities:', report.totals.zeroCitationEntities);
  console.log('  Top priority:', report.top100HighestPriority[0]?.slug || 'none');
}

main();
