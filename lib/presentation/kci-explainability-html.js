...
const { escapeHtml, renderPublicationList } = require('./citation-presentation.js');
...
function renderComponentBlock(key, label, component) {
  ...
  if (key === 'citation' && component.available && component.publications?.length) {
    extra.push(renderPublicationList(component.publications, { listClass: 'kci-publications' }));
  } else if (key === 'citation' && component.available && component.publicationTitles?.length) {
    extra.push(
