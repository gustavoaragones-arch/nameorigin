...
} = require('./citation-presentation.js');
...
function buildKciExplainabilityModel(input) {
  ...
  const publicationReferences = resolvePublicationReferencesFromRecord(
    citationRecord,
    citationRegistryIndex,
  );
  const publicationTitles = publicationReferences.map((row) => row.formatted);
  ...
  return {
    ...
    components: {
      ...
      citation: {
        ...
        citationCount: publicationReferences.length,
        publicationTitles,
...
function buildExplainabilityForSlug(slug, ctx) {
  ...
  return buildKciExplainabilityModel({
    ...
    citationRecord,
    popularityRecord,
    citationRegistryIndex: ctx.citationRegistryIndex,
